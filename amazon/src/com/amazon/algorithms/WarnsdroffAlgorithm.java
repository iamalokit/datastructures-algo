...
public class WarnsdroffAlgorithm {
	...
	public static void main(String [] args) {
		while(!new WarnsdroffAlgorithm().findClosedTour()) {
			;
		}
	}
	...
	private boolean neighbour(int x, int y, int xx, int yy) {
		for(int i = 0; i < N; ++i) {
			if(((x + cx[i] == xx) && ((y+cy[i]) == y)))
				return true;
		}
		return false;
	}
	...
	private Cell nextMove(int[] a, Cell cell) {
		...
		int start = ThreadLocalRandom.current().nextInt(10000) % N;
	...
	private boolean isempty(int[] a, int x, int y) {
